...
def SetVal(key, value, state):
    (err_msg, state1) = ensure_connected(state)
    if err_msg:
        return (err_msg, state1)
    res = state1['stub'].SetVal(pb2.keyValue(key = key, value = value))    
    res = str(res)
    res = res.split()
    print(res[1])
...
def GetVal(key):
    ...
    if err_msg:
        return (err_msg, state1)
    res = state1['stub'].GetVal(pb2.key(key = key))    
    res = str(res)
    res = res.split()
    print(res[-1])
...
def exec_cmd(line, state):
    ...
    if parts[0] == 'connect':
        ...
        if(len(parts)>2):
          return SetVal(str(parts[1]), str(parts[2]), state)
