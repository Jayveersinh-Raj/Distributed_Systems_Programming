...
class Handler(pb2_grpc.RaftNodeServicer):
    def RequestVote(self, request, context):
        ...
        with state_lock:
            ...
            if state['commitIndex'] > last_log_index:
                become_a_follower()
                reply = {'result': False, 'term': state['term']}

            if state['commitIndex'] > 0 and state['term']!= last_log_term:
                become_a_follower()
                reply = {'result': False, 'term': state['term']}
    ...
    def AppendEntries(self, request, context):
        ...
        with state_lock:
            ...
            if request.entry in state['logs']:
                state['logs'].append(request.entry)
                state['logs'] = list(dict.fromkeys(state['logs']))
    ...
    def SetVal(self, request, context):
      try: 
        state['logs'].append((request.key, request.value))
        return pb2.status(status = True)
      except:
        return pb2.status(status = False)
    
    def GetVal(self, request, context):
        key_list = [i[0] for i in state['logs']]
        try:
          index = key_list.index(request.key)
          return pb2.valueStatus(status = True, value = state['logs'][index][1])
        except:
          return pb2.valueStatus(status = False, value = "No such key found")
